...
def get_page_info_in_this_journal(dr, journal, start_page, start, end, journal_path):
    ...
    for this_page in range(start_page, pages_number + 1):
        ...
        if req == -1:
            log.logger.debug('期刊:' + journal + ',第' + str(this_page) + '页爬取失败')
            filesys.write_file('./error/long_error2.csv', (journal, start, end, this_page))
            return current_total_get
...
def get_long_error(journal, start_time, end_time, journal_path):
    time.sleep(1)
    deal_file_path = "./error/long_error2.csv"
    dr = create_driver()
    try:
        dr.get(login_url)
    except:
        time.sleep(3)
        filesys.write_file(deal_file_path, (journal, start_time, end_time, 1))
        dr.quit()
        return
    this_journal_get_num = get_page_info_in_this_journal(dr, journal, 1, start_time,
                                                         end_time, journal_path)
    if this_journal_get_num == -1:
        filesys.write_file(deal_file_path, (journal, start_time, end_time, 1))
        dr.quit()
    else:
        log.logger.debug('期刊' + journal + start_time + "-" + end_time + '爬取完成,共爬取文献：' + str(
            this_journal_get_num))
        dr.quit()


def get_long_error2(count):
    deal_file_path = './error/long_error2.csv'
    long_error2_list = read_long_error2_list(deal_file_path)
    while long_error2_list:
        count -= 1
        date = time.strftime("%Y-%m-%d_%H-%M-%S")
        log_path = './error/long_error2_log/' + date + "long_error2.csv"
        shutil.move(deal_file_path, log_path)
        filesys.create_file(deal_file_path, ['期刊名', '开始时间', "结束时间", "页号"])
        for long_error2 in long_error2_list:
            dr = create_driver()
            try:
                dr.get(login_url)
            except:
                time.sleep(30)
                filesys.write_file(deal_file_path, (long_error2[0], long_error2[1], long_error2[2], long_error2[3]))
                dr.quit()
                continue
            cu_jo = long_error2[0].replace(":", "-")  # 期刊名中一些特殊符号在创建文件的时候替换
            cu_jo = cu_jo.replace("/", "-")
            journal_path = "./data/" + cu_jo + ".csv"
            this_journal_get_num = get_page_info_in_this_journal(dr, long_error2[0], int(long_error2[3]), long_error2[1],
                                                                 long_error2[2], journal_path)
            if this_journal_get_num == -1:
                filesys.write_file(deal_file_path, (long_error2[0], long_error2[1], long_error2[2], long_error2[3]))
                dr.quit()
                continue
            else:
                log.logger.debug('期刊' + long_error2[0] + long_error2[1] + "-" + long_error2[2] + '爬取完成,共爬取文献：' + str(
                    this_journal_get_num))
                dr.quit()
        long_error2_list = read_long_error2_list(deal_file_path)
        if not long_error2_list or count == 0:
            break


def get_long_journal():
    deal_file_path = './error/long_error.csv'
    long_error_list = read_long_error_list(deal_file_path)
    for long_error in long_error_list:
        cu_jo = long_error[0].replace(":", "-")  # 期刊名中一些特殊符号在创建文件的时候替换
        cu_jo = cu_jo.replace("/", "-")
        journal_path = "./data/" + cu_jo + ".csv"
        filesys.create_file(journal_path, ["序号", "篇名", "作者", "刊名", "发表时间", "被引", "自引", "外引", "文献总数"])
        get_long_error(long_error[0], "2019-01-01", "2019-04-30", journal_path)
        # get_long_error(long_error[0], "2020-01-01", "2020-12-31", journal_path)
...
def read_long_error_list(deal_file_path):
    read_long_error = list()
    with open(deal_file_path, 'r', encoding='gb18030', errors='ignore') as f:
        reader = csv.reader(line.replace('\0', '') for line in f)
        for row in reader:
            if not row:
                continue
            if row[0] not in '期刊名':
                read_long_error.append((row[0], ))
    return read_long_error


def read_long_error2_list(deal_file_path):
    read_long_error2 = list()
    with open(deal_file_path, 'r', encoding='gb18030', errors='ignore') as f:
        reader = csv.reader(line.replace('\0', '') for line in f)
        for row in reader:
            if not row:
                continue
            if row[0] not in '期刊名':
                read_long_error2.append((row[0], row[1], row[2], row[3]))
    return read_long_error2
